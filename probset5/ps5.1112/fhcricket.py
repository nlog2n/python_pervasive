...
class Beacon:
    ...
    def __init__(self,name="",dist=0,id=""):
        ...
        self.max_reading  = 20     # at most 20-reading buffer
    ...
    def new_reading(self, reading):
        ...
        self.readings = self.remove_old_readings()
...
#    def __init__(self,host="172.18.182.217",port=2947):
  ...
  #    Assumes the speed of sound is 344.49 m/s
    ...
    def triangulate(self,b1,b2) :
          ...
          proportional  = 2
          d1 = b1.get_newest_dist()
          d2 = b2.get_newest_dist()
          ...
          w =  b2.x
          ret_x =  int(((w*w + d1*d1 - d2*d2)/(2*w))) / proportional
          tmp = d1*d1 - ret_x * ret_x 
          if tmp>=0 :
             ret_y =  int(math.sqrt(tmp )) / proportional
          else :
             return None

          if  math.fabs(self.old_x -ret_x) + math.fabs(self.old_y - ret_y) < 20 :
              return None
