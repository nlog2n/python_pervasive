...
if  WINDOWS_VERSION is not True:
...
class gui:
   ...
   def __init__(self):
      ...
      # ( galaudio use ox port, see portmap )#
   ...
   def on_drawingarea1_button_press_event(self,widget,event):
        ...
        if not self.edit_mode :
           ...
           if event.button == 1 : # left button click to select points
               ...
               if v is not None :
                   if  self.srcpoint is None and self.destpoint is None :
                                   self.srcpoint = v.name
                                   self.path = None
                                   print "select starting point "                                   
                   elif  self.srcpoint is not None and self.destpoint is None :
                                   if  v.name != self.srcpoint :
                                          ...
                                          self.srcpoint  = None
                                          print "de-select starting point "
                   elif  self.srcpoint is None and self.destpoint is not None :
                                   if  v.name != self.destpoint :
                                          ...
                                          self.destpoint = None
                                          print "de-select ending point "                                          
                   elif  self.srcpoint is not None and self.destpoint is not None :
                                   if v.name == self.srcpoint  or v.name == self.destpoint:
                                          self.srcpoint = None
                                          self.destpoint = None
                                          self.path = None
                                   else  :
                                          self.srcpoint = v.name
                                          self.destpoiint = None
                                          self.path = None
                                          print "select starting point "                                   
...
#getAttribute(name)
   ...
   def  feed(self) :
                   ...
                   if  self.server.connected == False :   return
                   
                   
                   c = self.server.handleMessage(0)        
                   if c:           
